...
import leetcode_challenges.april.day21.BinaryMatrix

object LC0_LeftmostColumnWithAtLeastAOne {
  ...
  def leftMostColumnWithOne(binaryMatrix: BinaryMatrix): Int = {
    if(binaryMatrix.dimensions() == null ||
      ...
      binaryMatrix.dimensions().get(0) == 0) return -1

    val row = binaryMatrix.dimensions().get(0)
    val col = binaryMatrix.dimensions().get(1)
